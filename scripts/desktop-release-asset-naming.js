...
const NORMALIZED_RELEASE_PATTERN =
  /^CineHarbor\.Desktop_(?:(.+?)_)?(macos|windows|linux)-(x64|arm64)(\.dmg(?:\.sig)?|\.app\.tar\.gz(?:\.sig)?|-setup\.exe(?:\.sig)?|\.msi(?:\.sig)?|-portable\.zip|\.AppImage|\.deb|\.rpm)$/;
const LEGACY_RELEASE_PATTERN =
  /^CineHarbor\.Desktop_(?:(.+?)_)?(aarch64|arm64|x64|x86_64)(\.dmg(?:\.sig)?|\.app\.tar\.gz(?:\.sig)?|-setup\.exe(?:\.sig)?|\.msi(?:\.sig)?|-portable\.zip|_portable\.zip|\.AppImage|\.deb|\.rpm)$/;
...
function splitLabeledAssetName(assetName) {
  ...
  const displayMarker = `${displaySeparator}CineHarbor.Desktop_`;
  ...
  const dottedMarker = '.-.CineHarbor.Desktop_';
...
function buildNormalizedBareReleaseAssetName(assetName, releaseVersion) {
  ...
  if (normalizedMatch) {
    ...
    return {
      normalizedName: `CineHarbor.Desktop_${
  ...
  return {
    normalizedName: `CineHarbor.Desktop_${version}_${platform}-${architecture}${suffix}`,
    platform,
    architecture,
  };
}
...
function buildNormalizedReleaseAssetDescriptor({ assetName, releaseVersion }) {
  ...
  if (!bareName.startsWith('CineHarbor.Desktop_')) {
    return null;
  }
