...
- 操作：将 `CineHarbor/cineharbor-desktop` 已应用的 Agnir 操作包升级到最新稳定发布。
...
- 保留项：`project.identity`（`urn:cineharbor:project:cineharbor-desktop`）、四个 memory locator、`.agnir/*` 持久内存、`agnir/repository` 扩展及无关 README/`AGENTS.md` 内容均保留。
- 变更项：
