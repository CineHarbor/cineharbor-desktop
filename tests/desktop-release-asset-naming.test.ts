...
describe('desktop release asset naming', () => {
  ...
  it('builds safe normalized release filenames for updater urls', () => {
    expect(
      assetNamingModule.buildNormalizedReleaseAssetFileName({
        assetName: 'CineHarbor.Desktop_200.0.1_x64-setup.exe',
        releaseVersion: '200.0.1',
      })
    ).toBe('CineHarbor.Desktop_200.0.1_windows-x64-setup.exe');

    expect(
      assetNamingModule.buildNormalizedReleaseAssetFileName({
        assetName: 'Windows x64 - CineHarbor.Desktop_200.0.1_windows-x64-setup.exe',
        releaseVersion: '200.0.1',
      })
    ).toBe('CineHarbor.Desktop_200.0.1_windows-x64-setup.exe');
  ...
  it('adds a human-readable platform label to normalized release assets', () => {
    ...
    ).toBe('Windows x64 - CineHarbor.Desktop_200.0.1_windows-x64-setup.exe');

    expect(
      assetNamingModule.buildNormalizedReleaseAssetName({
        assetName: 'CineHarbor.Desktop_200.0.1_aarch64.dmg',
        releaseVersion: '200.0.1',
      })
    ).toBe('macOS Apple Silicon - CineHarbor.Desktop_200.0.1_macos-arm64.dmg');
  });
