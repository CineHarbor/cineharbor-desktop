...
const LOCAL_SERVICE_BINARY_NAME: &str = "cineharbor-local-service";
const LOCAL_SERVICE_CONFIG_FILE_NAME: &str = "desktop.config.json";
const LOCAL_SERVICE_DB_FILE_NAME: &str = "cineharbor-desktop.sqlite3";
...
fn compile_time_updater_pubkey() -> Option<String> {
    normalize_compile_time_value(option_env!("CINEHARBOR_UPDATER_PUBKEY"))
}
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| "info,cineharbor_desktop_shell=info".into()),
        )
        .with_target(false)
        .without_time()
        .init();
    ...
    if untracked_health.healthy {
        ...
        if !terminate_untracked_local_service(LOCAL_SERVICE_PORT)? {
            ...
            return Err(anyhow::anyhow!(
                "detected an untracked local service on {base_url} with version {detected_version}; current app expects {current_version}. Please fully quit CineHarbor and retry."
            ));
        }
...
async fn fetch_local_profile_sync_status(
    ...
    let response = reqwest::Client::new()
        ...
        .header("X-CineHarbor-Local-Token", access_token)
...
async fn run_local_service_diagnostics_impl(
    ...
    let mut log_lines = vec![
        "CineHarbor Desktop Local Service Diagnostics".to_string(),
...
async fn upload_local_service_diagnostics_impl(
    ...
    let payload = LocalServiceDiagnosticsUploadRequest {
        source_app: "cineharbor-desktop".to_string(),
        app_version: app_version.clone(),
        target_triple: env!("CINEHARBOR_TARGET_TRIPLE").to_string(),
    ...
    let response = reqwest::Client::new()
        ...
        .header(
            ...
            format!("CineHarbor-Desktop/{app_version}"),
        )
...
fn append_app_context_log_lines(app: &AppHandle, log_lines: &mut Vec<String>) {
    ...
    push_log_kv(log_lines, 1, "Application", "CineHarbor Desktop");
    push_log_kv(
        log_lines,
        1,
        "Version",
...
fn is_local_service_process_name(process_name: &str) -> bool {
    matches!(
        process_name,
        "cineharbor-local-service" | "cineharbor-local-service.exe"
    )
}
...
mod tests {
    ...
    fn finds_desktop_release_manifest_asset_url() {
        ...
        assert_eq!(
            find_desktop_release_manifest_url(Some(&[GithubReleaseAssetPayload {
                name: Some("CineHarbor.Desktop_200.0.0_x64-setup.exe".to_string()),
    ...
    async fn local_profile_sync_status_sends_access_token_header() {
        ...
        let server = tokio::spawn(async move {
            ...
            assert!(request.contains("x-cineharbor-local-token: test-access-token"));
            stream
